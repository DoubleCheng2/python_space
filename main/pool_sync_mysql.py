...
def work(item):
    datetime_column_name = item[2].split(',')[-2]
    config = dict()
    ...
    target_db = mdb.connect(host='', port='', user='', passwd='', charset='')
    target_cur = target_db.cursor()
    try:
        # 定义查询语句
        print('所运行的代码:', 'select %s from %s  where  %s >="%s" ;  ' % (
        item[2], item[1], datetime_column_name, item[3].strftime("%Y-%m-%d %H:%M:%S")))
        source_cur.execute('select %s from %s  where %s >="%s" ' % (
        item[2], item[1], datetime_column_name, item[3].strftime("%Y-%m-%d %H:%M:%S")))
        i = 2000
        while True:
            ...
            if not getstgdata:
                ...
                target_db.commit()
        update_sql = "update TD.synchtodate_new  set start_date =(select ifnull((select DATE_ADD(%s,INTERVAL -3 HOUR)  from %s where id=(select  max(id) from %s)),CURRENT_DATE))   where type='table' and target_table='%s'" % (
        datetime_column_name, item[6], item[6], item[6])
...
# 目标地址的数据连接
config_db = mdb.connect(host='', port=3306, user='', passwd='', db='', charset='')
config_cur = config_db.cursor()
# 从配置表中查询出来要同步的数据
config_cur.execute("select * from  TD.synchtodate_new where type='table' and status=1")
